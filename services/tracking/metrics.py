...
def sync_metrics_update(context):
    ...
    if hasattr(context, "video_processor"):
    ...
    last_saved_sets = st.session_state.get("last_saved_sets_completed",0)  # if key does not exist means the user logged first time and its last... is 0
    ...
    if not pose_detected and st.session_state.get("voice_pipeline"):
        result = st.session_state.voice_pipeline.process_event(
            event = "no pose detected",
            exercise = exercise,
            metrics = {"issue":"No pose detected! Please step into camera frame."}
        )  
