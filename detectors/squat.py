...
class SquatDetector(BaseExercise):
    ...
    def process(self,landmarks):
        ...
        if left_vis >= right_vis: # if left visibility is more clear then find angle of knee from left knee
            knee_angle = left_knee_angle
            hip_idx, knee_idx, ankle_idx, shoulder_idx = self.LEFT_HIP , self.LEFT_KNEE, self.LEFT_ANKLE, self.LEFT_SHOULDER # take best visibilty indices
        else:
        ...
        # check visibilty
        key_landmark_visibilty = landmarks[hip_idx].visibility >= self.MIN_VISIBILITY and landmarks[knee_idx].visibility >= self.MIN_VISIBILITY and landmarks[ankle_idx].visibility >= self.MIN_VISIBILITY
